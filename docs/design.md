...
- v2 범위: **① 강의실 의자 앉기/일어서기 ② 화면 공유/발표(강사 화면+마이크)**
- v2에서 제외(→v3 후보): 수강생 음성 대화, 계정/영속 저장(DB), 룸 확장, 라운지 소파 착석
...
### 15.5 비용·버전 (착수 전 리서치로 확정)
LiveKit Cloud 무료 티어로 시작, 사용량 과금은 발표 시간에만 발생. 정확한 무료 한도·단가·강의당 비용·SDK 버전은 구현 착수 직전 리서치(진행 중)로 검증하여 본 절에 반영한다.
