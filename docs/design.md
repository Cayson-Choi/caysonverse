...
**알려진 한계 (v1 수용):**
- 클라이언트 IP는 프록시 헤더(x-forwarded-for 등)로만 얻을 수 있어, **프록시 없는 환경(로컬 등)에서는 시도 제한이 전역 카운터로 동작**한다. 이 경우 공격자가 분당 5회 오답을 보내면 진짜 강사도 로그인이 잠길 수 있다. **Railway는 프록시 뒤에서 동작하므로 실배포에서는 IP별 제한이 정상 작동**한다 — 배포 문서에 명시할 것.
- IP를 얻을 수 없는 환경에서 강퇴 denySet은 닉네임 키만 남으므로, 강퇴당한 사용자가 닉네임을 바꾸면 재입장이 가능하다 (계정 없는 v1의 구조적 한계).
